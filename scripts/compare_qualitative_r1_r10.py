#!/usr/bin/env python3
"""Compare qualitative text-to-image R1-R10 retrieval results for two DM-Adapter checkpoints."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate two DM-Adapter checkpoints on the full test split, rank every caption "
    ...
    parser.add_argument(
        ...
        default="auto",
        help="DM-Adapter official retrieval mode. auto resolves to global.",
    )
    ...
    parser.add_argument("--num_experts", type=int, default=None, help="Override DM-Adapter MoE expert count.")
    parser.add_argument("--topk", type=int, default=None, help="Override DM-Adapter MoE top-k.")
    parser.add_argument("--reduction", type=int, default=None, help="Override DM-Adapter adapter reduction.")
...
def build_eval_args(cli_args: argparse.Namespace) -> SimpleNamespace:
    ...
    if cli_args.num_experts is not None:
        cfg["num_experts"] = int(cli_args.num_experts)
    if cli_args.topk is not None:
        cfg["topk"] = int(cli_args.topk)
    if cli_args.reduction is not None:
        cfg["reduction"] = int(cli_args.reduction)
    ...
    cfg.setdefault("num_experts", 6)
    cfg.setdefault("topk", 2)
    cfg.setdefault("reduction", 8)
...
def call_model_encoder(encoder: Any, tensor: torch.Tensor, kind: str) -> torch.Tensor:
    ...
    # DM-Adapter's MoE encoders thread l_aux through the transformer; zero is neutral for inference.
...
def embedding_cache_metadata(
    ...
    return {
        ...
        "num_experts": int(getattr(model_args, "num_experts", 6)),
        "topk": int(getattr(model_args, "topk", 2)),
        "reduction": int(getattr(model_args, "reduction", 8)),
